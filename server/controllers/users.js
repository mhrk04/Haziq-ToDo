...
router.post("/login", async (req, res) => {
  try {
    ...
    // Generate JWT
    jwt.sign(
      { id: userFound._id, username: userFound.username }, // <-- IMPORTANT: Include 'username' and 'id' here
      SECRET_KEY,
      { expiresIn: "3h" },
      (err, token) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ msg: "Failed to login", error: "Token generation error" });
        }
        return res.json({
          token,
          username: userFound.username, // <-- IMPORTANT: Explicitly send username
          msg: "Login Successfully",
        });
      }
    );
  } catch (e) {
    return res.status(400).json({ msg: "Failed to login", error: e.message });
  }
});
